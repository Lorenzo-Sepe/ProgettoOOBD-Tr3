...
public class AggiungiContattoFrame extends JFrame {
...
//	}
	...
	public AggiungiContattoFrame(Controller controller, JFrame Chiamante) {
		frame = this;
		c=controller;
		...
		labelFoto.setIcon(new ImageIcon(AggiungiContattoFrame.class.getResource("/Immagini/NoImage.jpg")));
						...
						btnConfirm.addMouseListener(new MouseAdapter() {
							...
							public void mouseClicked(MouseEvent e) {
								...
								try {
									...
									if(modelIndirizzi.getRowCount() >0) {
									...
									for (int i = 0; i < modelIndirizzi.getRowCount(); i++) {
										if (indexDelIndirizzoPrincipale == i) {
											c.aggiungiIndirizzo(idContatto, modelIndirizzi.getValueAt(i, 0).toString(),
													modelIndirizzi.getValueAt(i, 1).toString(),
													modelIndirizzi.getValueAt(i, 2).toString(),
													modelIndirizzi.getValueAt(i, 3).toString(),
													modelIndirizzi.getValueAt(i, 4).toString(), true);
										}
										else
											c.aggiungiIndirizzo(idContatto, modelIndirizzi.getValueAt(i, 0).toString(), 
													modelIndirizzi.getValueAt(i, 1).toString(),
													modelIndirizzi.getValueAt(i, 2).toString(),
													modelIndirizzi.getValueAt(i, 3).toString(),
													modelIndirizzi.getValueAt(i, 4).toString(), false);
									}
									// Load degli account 
									String accountFornitoreAccount;
									String accountNicknameAccount;
									String accountMailAccount;
									String accountBenvenutoAccount;
									for (int i = 0; i < modelAccounts.getRowCount(); i++) {
										accountFornitoreAccount = modelAccounts.getValueAt(i, 0).toString();
										accountNicknameAccount = modelAccounts.getValueAt(i, 1).toString();
										accountMailAccount = modelAccounts.getValueAt(i, 2).toString();
										accountBenvenutoAccount = modelAccounts.getValueAt(i, 3).toString();
										c.aggiungiAccount(idContatto, accountNicknameAccount, accountFornitoreAccount, accountBenvenutoAccount, accountMailAccount);
									...
									if (result == JOptionPane.OK_OPTION) {
										...
										for (int i = 0;i <tableAccounts.getRowCount();i++) {
											if (mailSelected.equals(tableAccounts.getValueAt(i,2))) {
												result2 = JOptionPane.showConfirmDialog(null,"Questa Email ̬ presente nell'account "+tableAccounts.getValueAt(i, 1)+ ", procedere ugualmente?","Warning",JOptionPane.OK_CANCEL_OPTION);

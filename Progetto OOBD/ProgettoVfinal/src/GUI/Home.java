		...
		mntmCreaCassaforte.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (c.getCassaforte()==null) {
					...
					JOptionPane.showMessageDialog(null, "La cassaforte � gi� stata creata", "ERROR", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
						...
						if (provaPassword.compareTo(c.getCassaforte().getPassword())==0) {
							...
							if (nuovaPassword != null) {
								if (nuovaPassword.compareTo(provaPassword) == 0) {
									JOptionPane.showMessageDialog(null, "La nuova password � uguale alla vecchia", "ERROR", JOptionPane.ERROR_MESSAGE);
								}
...
//										c.transactionCommit();
		...
		btnCercaContatto.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				JFrame searchFrame = new SearchFrame(controller, frame);
